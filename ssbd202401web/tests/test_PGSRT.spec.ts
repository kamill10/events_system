...
test('sorting_by_name_surname-test', async ({ page }) => {
    ...
    await page.locator('div.MuiBox-root:nth-child(2) > li:nth-child(1) > p:nth-child(1)').click();
    await page.locator('xpath=/html/body/div[1]/div[2]/div/div/div[1]/div[1]/div[1]/h5').click();
    await page.locator('#phrase').fill('Test');
    await page.locator('button.MuiButtonBase-root:nth-child(11)').click();
    const containsTestString = await page.waitForSelector('//td[contains(.,"Test")]');
    expect(containsTestString).not.toBeNull();
    await page.locator('#phrase').fill('Participant');
    await page.locator('button.MuiButtonBase-root:nth-child(11)').click();
    const containsParticipant = await page.waitForSelector('//td[contains(.,"Participant")]');
    expect(containsParticipant).not.toBeNull();
    await expect(page.getByRole('cell', { name: 'Manager' })).toBeHidden();
});