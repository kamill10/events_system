...
test('unblockAccount', async ({ page }) => {
  ...
  await expect(page.getByText('Account deactivated successfully!')).toBeVisible();
  ...
  // weryfikacja przypadku użycia
  await page.getByRole('tab', { name: 'Change profile details' }).click();
  await page.getByRole('checkbox').click();
  await page.getByRole('button', { name: 'Yes' }).click();
  await expect(page.getByText('Account activated successfully!')).toBeVisible();
  await page.getByRole('tab', { name: 'Profile details', exact: true }).click();
  await expect(page.getByRole('cell', { name: 'Yes', exact: true }).first()).toBeVisible();
});